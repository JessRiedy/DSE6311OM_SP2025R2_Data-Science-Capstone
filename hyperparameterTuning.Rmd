...
```{r Create_Model_RMSE_df, warning=FALSE}
...
# Function to add rows to the rmse_df
add_rmse_row <- function(df, model_name, train_rmse, test_rmse) {
  new_row <- tibble(
    Model = model_name,
...
#add the test and train RMSEs to the rmse_df
rmse_df <-  add_rmse_row(rmse_df, "Principal Component Regression", pcr_train_mse, pcr_test_mse)
...
#add the test and train RMSEs to the rmse_df
rmse_df <-  add_rmse_row(rmse_df, "Partial Least Squares Regression", plsr_train_mse, plsr_test_mse)
...
#add the test and train RMSEs to the rmse_df
rmse_df <- add_rmse_row(rmse_df, "Random Forest", rf_train_mse, rf_test_mse)
...
**Mean Square Error**
```{r mean_sqr_error, warning=FALSE}
mse <- data.frame(Model = c('Linear Regression (Elastic Net Regularization) - 1SE', 'Linear Regression (Elastic Net Regularization) - MIN', 'Principal Component Regression (PCR) - Test', 'Partial Least Squares Regression (PLSR)', 'Bagged Regression', 'Random Forest'), `MeanSquareError(MSE)` = c(mse_1se, mse_min, pcr_test_mse, plsr_test_mse, bagged_mse, rf_test_mse))

mse

ggplot(mse, aes(x = reorder(Model, MeanSquareError.MSE.), y = MeanSquareError.MSE.)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Mean Square Errors (MSE) of Each Model", x = "Model", y = "Mean Square Error") +
  theme_minimal() +
